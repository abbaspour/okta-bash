...
while getopts "e:t:d:A:i:f:k:hv?" opt
do
    case ${opt} in
        e) source ${OPTARG};;
...
readonly body=$(printf '{"iat": %s, "iss":"%s","sub":"%s","aud":"https://%s/oauth2/%s/v1/token","exp":%s, "jti": "%s"}' "${now}" "${client_id}" "${client_id}" "${AUTH0_DOMAIN}" "${authorization_server}" "${exp}" "${now}" | openssl base64 -e -A | sed s/\+/-/ | sed -E s/=+$//)
