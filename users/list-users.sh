...
function usage() {
  cat <<END >&2
USAGE: $0 [-e env] [-d domain] [-D preview] [-c client_id] [-a api_token] [-v|-h]
...
while getopts "e:a:d:D:hv?" opt; do
  case ${opt} in
  e) source "${OPTARG}" ;;
...
curl  -X GET -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: SSWS ${api_token}" "https://${yourOktaDomain}/api/v1/users"